...
private fun NavGraphBuilder.addRouteMain(navController: NavHostController) {
    composable(Screen.Main.route) {
        MainScreen(
            onClick = { event ->
                when (event) {
                    MainEvents.onClick(TypeClick.onClickLogin) -> {
                        navController.navigate(Screen.Login.route)
                    }
                    MainEvents.onClick(TypeClick.onClickRegister) -> {
...
private fun NavGraphBuilder.addRouteLogin(navController: NavHostController) {
    composable(Screen.Login.route) {
        ...
        LoginScreen(
            ...
            onClick = { event ->
                when (event) {
                    LoginEvents.onClick(TypeClick.onClickGoToRegister) -> {
                        navController.navigate(Screen.Register.route)
                    }
...
private fun NavGraphBuilder.addRouteRegister(navController: NavHostController) {
    composable(Screen.Register.route) {
        ...
        RegisterScreen(
            ...
            onClick = { event ->
                when(event){
                    RegisterEvents.onClick(TypeClick.onClickGoToLogin) -> {
                        navController.navigate(Screen.Login.route)
                    }
                    else -> {
                        registerViewModel.onEvent(event)
